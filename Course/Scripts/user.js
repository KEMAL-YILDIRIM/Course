    ...
    $("#tamamla").click(function() {
        $.ajax({
            ...
            success: function(data) {
                ...
                Cevaplar.push(Cevap1_clickedId, Cevap2_clickedId, Cevap3_clickedId, Cevap4_clickedId);

                for (var i = 1; i <= 4; i++) {

                    let bir = i - 1;
                    let iki = `Cevap${i}`;
                    if (Cevaplar[bir] === data[iki]) {
                        $("#" + Cevaplar[bir]).css("background", "green");
                    } else {
                        $("#" + Cevaplar[bir]).css("background", "red");
                    }
                }
            },
